...
from mq import RABBITMQ_URL, QUEUE_AUTO, publish_result, publish_done
        ...
        try:
            for round_num in range(50):
                ...
                for tc in tool_calls:
                    ...
                    await push({"tool_status": {"name": tool_name, "status": "executing"}})

                    if tool_name in tool_map:
                        try:
                            tool_result = await asyncio.to_thread(tool_map[tool_name].run, str(tool_input))
...
async def main():
    print("[AutoWorker] Connecting to RabbitMQ...")
    ...
    print(f"[AutoWorker] Listening on '{QUEUE_AUTO}'...")
    await auto_queue.consume(handle_auto_message)
    print("[AutoWorker] Ready.")
